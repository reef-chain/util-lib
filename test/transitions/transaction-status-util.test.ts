...
describe('parseAndRethrowErrorFromObserver', () => {
    ...
    it('should parse and throw an error', () => {
        testScheduler.run((helpers) => {
            ...
            expectObservable(observer.pipe(parseAndRethrowErrorFromObserver(observer, txIdent))).toBe(expected, null, new TxStatusError(error.message, txIdent));
        });
    });
});
...
describe('getNativeTransactionStatusHandler$', () => {
    ...
    it('should emit TxStage.INCLUDED_IN_BLOCK when status is in block', () => {
        ...
        expect(status$.getValue()).toEqual({ txStage: TxStage.INCLUDED_IN_BLOCK, txData: { blockHash: 'some-block-hash' }, txIdent });
    });
    ...
    it('should emit TxStage.BLOCK_FINALIZED when status is finalized', () => {
        ...
        expect(status$.getValue()).toEqual({ txStage: TxStage.BLOCK_FINALIZED, txData: { blockHash: 'some-block-hash' }, txIdent });
...
describe('getEvmTransactionStatus$', () => {
    ...
    beforeAll(async () => {
        ...
        const ethProvider = new ethers.providers.JsonRpcProvider('https://your-ethereum-node-endpoint');
    ...
    it('should emit transaction status events', async () => {
        ...
        const status$ = getEvmTransactionStatus$(sentTx.wait(), rpcApi, sentTx.hash);
