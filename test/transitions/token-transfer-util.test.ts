...
describe('nativeTransferSigner$', () => {
  it('should return an Observable', () => {
    ...
    const result = nativeTransferSigner$(MOCK_AMOUNT, mockSigner, MOCK_TO_ADDRESS);
  ...
  it('should return an Observable', () => {
    const mockProvider = {
      api: {
        query: {
          system: {
            account: jest.fn(() => Promise.resolve({ data: { free: '2000000000000000000' } })),
          },
        },
        tx: {
          balances: {
    ...
    const mockSigner = {};
    const result = nativeTransfer$(MOCK_AMOUNT, 'mockFromAddress', MOCK_TO_ADDRESS, mockProvider, mockSigner);
    expect(result).toBeInstanceOf(Observable);
  });
});
...
describe('reef20Transfer$', () => {
  it('should return an Observable', () => {
    ...
    const result = reef20Transfer$(MOCK_TO_ADDRESS, mockProvider, mockTokenAmount, mockTokenContract);
    expect(result).toBeInstanceOf(Observable);
  });
});