import {
  toTxErrorCodeValue
} from '../../src/transaction/txErrorUtil';
...
describe('toTxErrorCodeValue', () => {
  ...
  it('should return ERROR_MIN_BALANCE_AFTER_TX code for revert message', () => {
    ...
    expect(result.code).toEqual(TX_STATUS_ERROR_CODE.ERROR_MIN_BALANCE_AFTER_TX);
    expect(result.message).toEqual('You must allow minimum 60 REEF on account for Ethereum VM transaction even if transaction fees will be much lower.');
  });
  ...
  it('should return ERROR_UNDEFINED code for other message', () => {
    ...
    expect(result.message).toEqual('Transaction error: Some other error message');
  });
});