...
describe('decodePayloadMethod', () => {
  ...
  it('should decode method data for a contract method', async () => {
    ...
    const abi = [ { "name": "balanceOf", "inputs": [ { "name": "account", "type": "address" } ], "outputs": [ { "name": "", "type": "uint256" } ], "stateMutability": "view", "type": "function" } ];
    const decodedMethodData = await decodePayloadMethod(provider, methodDataEncoded, abi);

    expect(decodedMethodData.methodName).toEqual('balanceOf(address)');
    expect(decodedMethodData.args).toEqual(['0x123...']);
    expect(decodedMethodData.info).toEqual('Get the account balance of another account with address _owner.');
    ...
    expect(decodedMethodData.vm.evm.decodedData.params[0].toString()).toEqual('0x123...');
  });
});