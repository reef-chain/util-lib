...
describe('accountsWithUpdatedIndexedData$', () => {
  it('should emit the expected result', () => {
    ...
    const accountsWithChainBalance = { data: [{ data: { address: '0x123' } }], getStatusList: () => [] };
    const locallyUpdated = { data: [{ data: { address: '0x456', isEvmClaimed: false, evmAddress: null } }], getStatusList: () => [] };
    const indexed = { data: [{ address: '0x789', evm_address: null }], getStatusList: () => [] };

    // Define the expected output of the function
    const expectedOutput = {
      data: [{ data: { address: '0x123', isEvmClaimed: false, evmAddress: null } }],
      getStatusList: () => []
    };
