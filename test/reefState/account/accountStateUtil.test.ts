...
describe('getSignersToUpdate', () => {
    ...
    const signers = [{ address: 'address1', name: 'Signer 1' }, { address: 'address2', name: 'Signer 2' }, { address: 'address3', name: 'Signer 3' },];
...
describe('updateSignersEvmBindings', () => {
  it('should return an empty array if signers is empty', async () => {
    ...
    const signers = [];
    const result = await updateSignersEvmBindings(updateActions, provider, signers);
    expect(result).toEqual([]);
  });

  it('should update signers EVM bindings', async () => {
    const updateActions = [
      { type: 'UPDATE', dataType: 'ACCOUNT_EVM_BINDING', data: { address: '0x123', evmAddress: '0xabc' } },
    ];
    const provider = {}; // Mock provider object
    const signers = [
      { data: { address: '0x123' } },
    ...
    const result = await updateSignersEvmBindings(updateActions, provider, signers);
    expect(result).toEqual([
      { data: { address: '0x123', isEvmClaimed: true }, statusCode: 'COMPLETE_DATA' },
      { data: { address: '0x456' }, statusCode: 'MISSING_INPUT_VALUES', message: 'ERROR: Can not get account signer.' },
    ]);
