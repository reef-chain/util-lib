...
describe('selectedAccountAddressChange$', () => {
  ...
  it('should emit distinct account address changes', () => {
    testScheduler.run(({ cold, expectObservable }) => {
      ...
      const status1 = { data: account1, status: 'COMPLETE_DATA', statusList: [] };
      const status2 = { data: account2, status: 'COMPLETE_DATA', statusList: [] };
      const status3 = { data: account3, status: 'COMPLETE_DATA', statusList: [] };
