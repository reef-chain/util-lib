...
describe('accountsLocallyUpdatedData$', () => {
  ...
  it('should update the account data when updateSignersSubj emits', async () => {
    ...
    const mockSignersInjected = [{ address: 'address1' }, { address: 'address2' }];
    ...
    jest.spyOn(global, 'Date').mockImplementation(() => ({ getTime: () => 123456 }));
    ...
    expect(mockUpdateSignersEvmBindings).toHaveBeenCalledWith(
      ...
      mockSignersInjected.map((s) => toFeedbackDM(s, FeedbackStatusCode.COMPLETE_DATA)),
    );
  ...
  it('should handle errors and emit a feedback object with the error message', async () => {
    ...
    expect(result).toEqual(toFeedbackDM([], FeedbackStatusCode.ERROR, mockError.message));
  });
});