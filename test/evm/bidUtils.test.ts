...
import { describe, it, expect, createFakeProvider, createFakeSigner } from 'vitest';
...
describe('bindEvmAddress', () => {
  ...
  it('should call the success handler if onTxChange is not provided', (done) => {
    ...
    setTimeout(() => {
      expect(alertStub).to.have.been.calledWith(`Success, Ethereum VM address is ${signer.evmAddress}. Use this address ONLY on Reef chain.`);
      alertStub.restore();
      done();
    }, 0);
  });
  ...
  it('should call the error handler if the claimDefaultAccount call fails', (done) => {
    ...
    const claimDefaultAccountStub = sinon.stub(signer.signer, 'claimDefaultAccount').rejects(error);
