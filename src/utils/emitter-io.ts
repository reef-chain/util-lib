...
export class Emitter {
  ...
  public connect(request?: ConnectRequest, handler?: () => void): Emitter {
    ...
    for (var k in defaultConnectOptions) {
      request[k] =
        "undefined" === typeof request[k]
          ? defaultConnectOptions[k]
          : request[k];
    }

    request.host = request.host.replace(/.*?:\/\//g, "");
    var brokerUrl = `${request.secure ? "wss://" : "ws://"}${request.host}:${
      request.port
    }`;
    ...
    this._mqtt.on("connect", () =>
      this._tryInvoke(EmitterEvents.connect, this)
    );
    this._mqtt.on("close", () =>
      this._tryInvoke(EmitterEvents.disconnect, this)
    );
    this._mqtt.on("offline", () =>
      this._tryInvoke(EmitterEvents.offline, this)
    );
    this._mqtt.on("error", error =>
      this._tryInvoke(EmitterEvents.error, error)
    );
   ...
   */
  public on<K extends keyof EmitterEventsArgsMap>(
    event: K,
    callback: (args: EmitterEventsArgsMap[K]) => void
  ): Emitter {
   ...
   */
  public off<K extends keyof EmitterEventsArgsMap>(
    event: K,
    callback: (args: EmitterEventsArgsMap[K]) => void
  ): Emitter {
    this._checkEvent("off", event);
  ...
  private _formatChannel(key: string, channel: string, options: Option[]) {
    ...
    if (key && key.length > 0)
      formatted = this._endsWith(key, "/")
        ? key + channel
        : key + "/" + channel;

    // Add trailing slash
    if (!this._endsWith(formatted, "/")) formatted += "/";
    ...
    if (options != null && options.length > 0) {
      ...
      for (var i: number = 0; i < options.length; ++i) {
        ...
        if (i + 1 < options.length) formatted += "&";
      }
    }
