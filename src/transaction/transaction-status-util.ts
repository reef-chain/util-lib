...
export function parseAndRethrowErrorFromObserver(
...
export function getEvmTransactionStatus$(
  evmTxPromise: Promise<any>,
  rpcApi: ApiPromise,
  txIdent: string
): Observable<TransactionStatusEvent> {
  const status$ = new Observable(observer => {
    evmTxPromise
      .then(tx => {
        observer.next({
          txStage: TxStage.BROADCAST,
          txData: tx,
          txIdent,
        ...
        tx.wait()
          .then(async receipt => {
            ...
            const unsubHeads = await rpcApi.rpc.chain.subscribeFinalizedHeads(
              lastHeader => {
                if (
                  receipt.blockHash.toString() === lastHeader.hash.toString()
                ) {
                  observer.next({
                    txIdent,
                ...
                if (--count < 0) {
                  if (count > finalizedCount) {
                    observer.next({
                      txIdent,
                      txStage: TxStage.BLOCK_NOT_FINALIZED,
                      txData: receipt,
                    });
                  }
                  unsubHeads();
...
export function getNativeTransactionStatusHandler$(txIdent: string): {
