...
> = new Map<string, Observable<StatusDataObject<Pool | null>>>();
// TODO listen to pool events and refresh then
const poolsRefresh$ = timer(0, 420000);

function isPoolCached(
  token1: Token | TokenBalance,
  token2: Token | TokenBalance
) {
  return (
    cachePool$.has(`${token1.address}-${token2.address}`) ||
    cachePool$.has(`${token1.address}-${token2.address}`)
  );
}
...
const getPool$ = (
  ...
  return (
    cachePool$.get(`${token1.address}-${token2.address}`) ||
    cachePool$.get(`${token2.address}-${token1.address}`)!
  );
};
...
export const fetchPools$ = (
  ...
  const poolsArr$: Observable<StatusDataObject<Pool | null>>[] = tokens
    .filter(tkn => tkn.data.address !== REEF_ADDRESS)
