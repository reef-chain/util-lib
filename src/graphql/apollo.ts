...
const apolloLinksClientInstance$: Observable<ApolloClient<any>> =
  splitLink$.pipe(
    map(
      splitLink =>
        new ApolloClient({
...
export const apolloClientInstance$: Observable<ApolloClient<any>> = merge(
  apolloLinksClientInstance$,
  apolloClientSubj
).pipe(distinctUntilChanged(), shareReplay(1));

export const zenToRx = <T>(zenObservable: ZenObservable<T>): Observable<T> =>
  new Observable(observer =>
    zenObservable.subscribe(
      v => observer.next(v),
      err => console.log("Apollo subscribe ERR=", err)
    )
  );
