...
export const attachPendingTxObservableSubj = new Subject<
...
export const pendingTxList$ = attachPendingTxObservableSubj.pipe(
  ...
  catchError(err => {
    ...
    return of({
      txIdent: err.txIdent,
      txStage: TxStage.ENDED,
    } as TransactionStatusEvent);
  }),
  scan(
    (
      statById: Map<string, TransactionStatusEvent>,
      newState: TransactionStatusEvent
    ) => {
      if (
        newState.txStage == TxStage.BLOCK_NOT_FINALIZED ||
        newState.txStage == TxStage.BLOCK_FINALIZED ||
        newState.txStage == TxStage.ENDED
      ) {
