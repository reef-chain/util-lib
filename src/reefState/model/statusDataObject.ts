...
export class StatusDataObject<T> {
  ...
  hasStatus(
    status: FeedbackStatusCode | FeedbackStatusCode[],
    propName?: string
  ): boolean {
...
function createFeedBackStatus(
  statCode: FeedbackStatusCode | undefined,
  message?: string,
  propName?: string
): FeedbackStatus {
  const code = statCode
    ? (statCode as FeedbackStatusCode)
    : FeedbackStatusCode.COMPLETE_DATA;
  return { code, message, propName };
}

// TODO test with different params
function createStatusFromCode(
  statCode?: FeedbackStatusCode | FeedbackStatusCode[] | FeedbackStatus[],
  message?: string,
  propName?: string
): FeedbackStatus[] {
  ...
  if (Array.isArray(statCode) && statCode.length) {
    if ((statCode as FeedbackStatus[])[0]?.code == null) {
      statCode.forEach(sc =>
        status.push(createFeedBackStatus(sc as FeedbackStatusCode))
      );
    } else {
    ...
    status.push(
      createFeedBackStatus(statCode as FeedbackStatusCode, message, propName)
    );
  }
...
export const toFeedbackDM = <T>(
  data: T,
  statCode?: FeedbackStatusCode | FeedbackStatusCode[] | FeedbackStatus[],
  message?: string,
  propName?: string
): StatusDataObject<T> => {
  return new StatusDataObject<T>(
    data,
...
export const collectFeedbackDMStatus = (
  items: StatusDataObject<any>[]
): FeedbackStatusCode[] => {
...
export const findMinStatusCode = (
  feedbackDMs: (StatusDataObject<any> | undefined)[]
): FeedbackStatusCode => {
  const statListArr = feedbackDMs.reduce(
    (
      stListArr: (FeedbackStatus | undefined)[],
      sdo: StatusDataObject<any> | undefined
    ) => {
  ...
  const minStat = statCodes.reduce(
    (s: FeedbackStatusCode, v: FeedbackStatusCode | undefined) => {
      if (v == null) {
...
export function skipBeforeStatus$<T>(
  observable: Observable<StatusDataObject<T>>,
  status: FeedbackStatusCode
): Observable<StatusDataObject<T>> {
