...
> = combineLatest([instantProvider$, availableAddresses$]).pipe(
  ...
  map(
    (
      ...
      if (isFeedbackDM(balancesAndSigners)) {
        return balancesAndSigners as StatusDataObject<
          StatusDataObject<ReefAccount>[]
        >;
      }
      const balAndSig = balancesAndSigners as {
        balances: any[];
        signers: ReefAccount[];
      };
      const balances_sdo: StatusDataObject<ReefAccount>[] = balAndSig.signers
        .map(sig => {
          ...
          if (
            bal &&
            (!sig.balance || !BigNumber.from(bal.balance).eq(sig.balance))
          ) {
            return {
              ...sig,
  ...
  catchError(err =>
    getAddressesErrorFallback(err, "Error chain balance=", "balance")
  ),
  shareReplay(1)
);
