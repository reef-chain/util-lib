...
export const selectedAddress$: Observable<string | undefined> =
  selectedAddressSubj
    .asObservable()
...
combineLatest([accounts_status$, selectedAddress$])
  ...
  .subscribe(
    ([signers, address]: [
      ...
      if (!saved) {
        const firstSigner =
          signers && signers.data && signers.data[0]
            ? signers.data[0].data
            : undefined;
        setSelectedAddress(saved || firstSigner?.address);
      }
    }
  );

export const selectedAccount_status$: Observable<
  StatusDataObject<ReefAccount> | undefined
> = combineLatest([selectedAddress$, accounts_status$]).pipe(
  map(
    (
      ...
      return foundSigner
        ? toFeedbackDM(
            { ...foundSigner.data } as ReefAccount,
