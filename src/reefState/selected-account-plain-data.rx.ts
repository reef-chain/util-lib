...
export const accounts$: Observable<ReefAccount[] | null | undefined> =
  unwrapSDOArray(accounts_status$);
export const selectedAccount$: Observable<ReefAccount | undefined | null> =
  unwrapSDO(selectedAccount_status$);
export const selectedTokenBalances$: Observable<
  (Token | TokenBalance)[] | null | undefined
> = unwrapSDOArray(selectedTokenBalances_status$);
export const selectedNFTs$: Observable<NFT[] | null | undefined> =
  unwrapSDOArray(selectedNFTs_status$);
...
function unwrapSDOArray<T>(
  sdoObservable: Observable<StatusDataObject<StatusDataObject<T>[]>>
): Observable<T[] | null | undefined> {
  return sdoObservable.pipe(
    map(res => {
      if (res.hasStatus(FeedbackStatusCode.COMPLETE_DATA)) {
      ...
      if (
        findMinStatusCode(res.data) < FeedbackStatusCode.MISSING_INPUT_VALUES
      ) {
        return undefined;
      }
      return null;
