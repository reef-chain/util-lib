...
export const sortReefTokenFirst = (
  tokens: StatusDataObject<Token | TokenBalance>[]
): StatusDataObject<Token | TokenBalance>[] => {
  const reefTokenIndex = tokens.findIndex(
    (t: StatusDataObject<Token | TokenBalance>) =>
      t.data.address === REEF_ADDRESS
  );
  if (reefTokenIndex > 0) {
    return [
...
export const toTokensWithPrice_sdo = ([tokens, reefPrice, pools]: [
  ...
  return toFeedbackDM(
    tknsWPrice,
    tknsWPrice.length
      ? collectFeedbackDMStatus(tknsWPrice)
      : tokens.getStatusList()
  );
};
