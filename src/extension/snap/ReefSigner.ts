...
export default class ReefSigner implements ReefInjectedSigner {
  ...
  private getResponseStatus(
    ...
    hasVM?: boolean,
    requestedVM: ReefVM = ReefVM.NATIVE
  ): ReefSignerResponse {
    if (selectedSigner) {
      if (hasVM) {
        return {
          data: selectedSigner,
          status: ReefSignerStatus.OK,
          requestedVM,
        };
      } else {
        return {
          data: undefined,
          status: ReefSignerStatus.SELECTED_NO_VM_CONNECTION,
          requestedVM,
        };
      }
      ...
      if (this.isSelectedAccountReceived && !this.selectedSignerAccount) {
        return {
          data: undefined,
          status: ReefSignerStatus.NO_ACCOUNT_SELECTED,
          requestedVM,
        };
      }
    }
    ...
    return {
      data: undefined,
      status: ReefSignerStatus.CONNECTING,
      requestedVM,
    };
  }

  private static createReefSigner(
    selectedSignerAccount?: InjectedAccount,
  ...
  private static async hasConnectedVM(
    ...
    return (
      !connectedVM ||
      (connectedVM === ReefVM.EVM && (await signer?.isClaimed()))
    );
  }
}
